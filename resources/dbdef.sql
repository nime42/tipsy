...
select d.*,r.rows FROM draws d LEFT JOIN (SELECT drawid,group_concat(rownr||';'||teams||';'||bet||';'||coalesce(result,'')||';'||coalesce(status,'')||';'||coalesce(matchstart,'')||';'||coalesce(matchtime,'')||';'||coalesce(lastevent,''),'|') AS rows FROM draw_rows GROUP BY drawid) r ON d.id=r.drawid;
