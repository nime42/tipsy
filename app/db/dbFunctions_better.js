...
function isPremium(userId, groupId) {
    let sql = "select is_premium from v_group_members where userid=? and groupId=?";
    const row = db.prepare(sql).get(userId, groupId);
    if (row !== undefined && row.is_premium === 1) {
        return true;
    } else {
        return false;
    }
...
function getStatistics(userId, groupId, callback = console.log) {
    ...
    for (key in stats) {
        ...
        r.average_topptips = parseFloat(Number(r.average_topptips).toFixed(2));
        ...
        r.average_stryktips = parseFloat(Number(r.average_stryktips).toFixed(2));
