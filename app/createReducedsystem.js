...
function main(argv) {
    let argDescr = " [stryktips|europatips|topptips] -row [SvF|Odds|1,X,2..] -maxErrors [0,1,2...] -singles [0,1,2..] -impossibles [0:X,1:2..] -outfile [filename] -max1 n -maxX n -max2 n -median min-max";
    if (argv.length < 3) {
        console.log("Usage: " + argv[1] + argDescr);
        return;
    }

    let optionList = ["-row", "-maxErrors", "-singles", "-impossibles", "-outfile", "-max1", "-maxX", "-max2", "-median"];
...
function parseOptions(argv, optionList) {
    ...
    if (args["-median"]) {
        let [min, max] = args["-median"].split("-");

        if (args["-median"].match(/^[0-9\.]*-[0-9\.]*$/) == null) {
            console.log("Median must be in the form min-max");
            process.exit(1);
        }

        params.median = [min !== "" ? Number(min) : 0, max !== "" ? Number(max) : 1];
    }
...
function removeHigherOdds(systems, draw, median) {
    const generateRows = (matches = 8) => {
        // Möjliga utfall för varje match
        const outcomes = ['1', 'X', '2'];

        let combinations = [];

        function generate(currentRow) {
            if (currentRow.length === matches) {
                combinations.push(currentRow);
                return;
            }
            for (let outcome of outcomes) {
                generate([...currentRow, outcome]);
            }
        }
    ...
    const getOdds = (draws) => {
        let odds = [];
        draws.forEach(d => {
            odds.push({ "1": Number(d.odds.one.replace(",", ".")), "X": Number(d.odds.x.replace(",", ".")), "2": Number(d.odds.two.replace(",", ".")) });
        });
        return odds;
    }

    const calcOdds = (row, odds) => {
        let totalOdds = 1;
        row.forEach((match, index) => {
            if (match === '1') {
                totalOdds *= odds[index]["1"];
            } else if (match === 'X') {
                totalOdds *= odds[index]["X"];
            } else {
                totalOdds *= odds[index]["2"];
            }
        });
        return totalOdds;
    }
    ...
    const allRows = generateRows(draw.draws.length)
    const allOdds = getOdds(draw.draws);
    let rowInfo = [];
    allRows.forEach(row => {
        rowInfo.push({ row: row, odds: calcOdds(row, allOdds) });
    });
    let sorted = rowInfo.sort((a, b) => a.odds - b.odds);
    let minIndex = Math.floor(sorted.length * median[0]);
    let maxIndex = Math.floor(sorted.length * median[1]);
    minIndex = Math.min(minIndex, sorted.length - 1);
    maxIndex = Math.min(maxIndex, sorted.length - 1);
    let minOdds = sorted[minIndex].odds;
    let maxOdds = sorted[maxIndex].odds;

    let filtered = systems.filter(s => (calcOdds(s.row.split(","), allOdds) >= minOdds && calcOdds(s.row.split(","), allOdds) <= maxOdds))
    console.log("Removed " + (systems.length - filtered.length) + " rows with odds than outside " + median);
    return filtered;
...
function makeReducedSystem(product, options) {
    matchInfoHandler.getPlayable(product, function (status, data) {
        if (status) {
            ...
            if (options.median) {
                systems.singleRows = removeHigherOdds(systems.singleRows, draw, options.median);
            }
